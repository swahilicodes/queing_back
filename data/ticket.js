...
router.get("/get_display_tokens", async (req, res, next) => {
  ...
  const isDiabeticValue =
    isDiabetic === "true" ? 1 : isDiabetic === "false" ? 0 : null;
  const isChildValue = isChild === "true" ? 1 : isChild === "false" ? 0 : null;
    ...
    try {
      const disabledToks = await Ticket.findAll({
        where: {
          ...
          stage: stage,
          [Op.or]: [{ status: "waiting" }, { serving: true }],
          disabled: true,
    ...
    try {
      const disabledToks = await Ticket.findAll({
        where: {
          ...
          disabled: true,
          createdAt: { [Op.gte]: twelveHoursAgo },
          clinic_code: { [Op.in]: clinics, paid: true },
        },
        limit: 3,
        order: [
          // ['disabled', 'DESC'],
...
router.get("/next_stage", async (req, res, next) => {
  ...
  if (mr_number.trim() === "") {
    ...
    axios
      .get(`http://${ip}/dev/jeeva_api/swagger/patients/${mr_number}`)
      .then((data) => {
...
router.post("/clinic_go", async (req, res, next) => {
  ...
  if (mr_number.trim() === "") {
    ...
    axios
      .get(`http://${ip}/dev/jeeva_api/swagger/consultation/${mr_number}`)
      .then(async (data) => {
        if (data.data.status === 201) {
...
router.put("/finish_token/:id", async (req, res, next) => {
  const id = req.params.id;
  const { stage, mr_number, penalized, sex, recorder_id, name, age, category } =
    req.body;
  const twelveHoursAgo = new Date(Date.now() - 12 * 60 * 60 * 1000);
  try {
    const ticket = await Ticket.findOne({
    ...
    if (!ticket) {
      ...
      if (tiki) {
        return res
          .status(400)
          .json({ error: `${mr_number} exists in ${stage}` });
      } else {
        await ticket.update({
          status: "waiting",
          serving: false,
